<?php
	...
	if(isset($_POST['commenter'])) {
		if(isset($_POST['comment']) AND !empty($_POST['comment'])) {
			...
			$ins = $bdd->prepare('INSERT INTO bc_lieu_commentaire (pseudo, commentaire, id_article, dateCommentaire) VALUES (?,?,?, NOW())');
			$ins->execute(array($_SESSION['nomUtilisateur'],$commentaire,$get_id));
		}
	}
	$commentaires = $bdd->prepare('SELECT * FROM bc_lieu_commentaire WHERE id_article = ? ORDER BY dateCommentaire DESC');
  $commentaires->execute(array($get_id));
	$photo = $bdd->prepare('SELECT * FROM bc_lieu_image_propose WHERE id_article = ? AND valider = 1');
  $photo->execute(array($get_id));
?>
<?php
	$reqVote = $bdd->prepare("SELECT AVG(vote) as somme FROM bc_vote_lieu WHERE id_lieu = ?");
...
<?php
	if(isset($_POST['partage'])) {
		if(!empty($_POST['recevoir'])) {
			...
			$header .= "\r\n";
      $message=''. 'http://192.168.128.13/~e180810/EVAL_V4/pageLieuDetail.php?id='. $get_id . ' <br> ' . $title . ' <br> ' . $description . ' <br> ' . $lieu . ' <br>  ' .'<img id="imageEve" src="img/'. $img .'.jpg" alt="representation de l\'événement" >'.'';
...
<?php
	if(isset($_POST['photoSend'])){
		if(isset($_FILES['miniature'])){
			...
			$nomImage = strtolower($nomImage).'li'.$get_id;
			$nomImage = str_replace(' ','',$nomImage);
			$insertImage = $bdd->prepare('INSERT INTO bc_lieu_image_propose(image,id_article,valider) VALUES (?,?,0)');
...
?>
<?php
	$updateView=$bdd->prepare("UPDATE bc_lieu SET nbConsultation = nbConsultation + 1 WHERE id_lieu = ?");
      ...
      <button class="follow" name="button">Suivre</button>
			<?php
				if(isset($_SESSION['id'])){
					if($_SESSION['admin']==1){		
			?>
			<a href="supprimer.php?id_lieu=<?=$get_id ?>" onclick="window.open(this.href, 'nom de ta fenêtre', 'height=400, width=400, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);">Supprimer</a>
			<a href="modifier.php?id_lieu=<?=$get_id ?>" onclick="window.open(this.href, 'nom de ta fenêtre', 'height=400, width=400, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);">Modifier</a>
			<?php 
					}
				}
			?>
    ...
    <section class="consult">
      <fieldset>
        <legend>Avis</legend>
				...
				<div class="vote">
          <a href="voteLieu.php?v=1&id=<?= $get_id ?>">☆</a>
          <a href="voteLieu.php?v=2&id=<?= $get_id ?>">☆</a>
          <a href="voteLieu.php?v=3&id=<?= $get_id ?>">☆</a>
          <a href="voteLieu.php?v=4&id=<?= $get_id ?>">☆</a>
          <a href="voteLieu.php?v=5&id=<?= $get_id ?>">☆</a>
        </div>
				<?php
					}
				?>
