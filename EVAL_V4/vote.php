...
if(isset($_GET['v'],$_GET['id'],$_SESSION['id']) AND !empty($_SESSION['id']) AND !empty($_GET['v']) AND !empty($_GET['id'])) {
   ...
   $check = $bdd->prepare('SELECT id_event FROM bc_event WHERE id_event = ?');
...
function addOrRemoveVote($getid,$sessionid,$getVoteValue,$bdd){
	$check_vote = $bdd->prepare('SELECT id_vote FROM bc_vote_event WHERE id_event = ? AND id_membre = ?');
  $check_vote->execute(array($getid,$sessionid));
  $deleteVote = $bdd->prepare('DELETE FROM bc_vote_event WHERE id_event = ? AND id_membre = ?');
  $deleteVote->execute(array($getid,$sessionid));
  if($check_vote->rowCount() == 1) {
		$deleteVote = $bdd->prepare('DELETE FROM bc_vote_event WHERE id_event = ? AND id_membre = ?');
    $deleteVote->execute(array($getid,$sessionid));
  } else {
    $insertVote = $bdd->prepare('INSERT INTO bc_vote_event (vote, id_membre,id_event) VALUES (?, ?,?)');
    $insertVote->execute(array($getVoteValue, $sessionid,$getid));
  }
	header('Location: http://192.168.128.13/~e180810/EVAL_V4/pageEvenement.php?id='.$getid);
